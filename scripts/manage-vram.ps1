...
$VRAM_APPS = @{
    ...
    # AI desktop apps (all GPU-accelerated WebView/Electron)
    ai_apps = @(
        @{Name="ChatGPT";       StartPath=""}  # Windows Store app
        @{Name="Codex";         StartPath=""}   # Windows Store app
        @{Name="claude";        StartPath=""}   # Windows Store app
        @{Name="Todoist";       StartPath=""}   # Windows Store app
    )
    # System tray / background GPU consumers
    tray = @(
        @{Name="XboxPcTray";    StartPath=""}
    )
    # Keep these but don't close: dwm, explorer, wezterm-gui, nvcontainer
    # (essential for display, terminal, and GPU functionality)
}
}
