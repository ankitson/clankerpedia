...
## ⚠️ RECURRING TASK RULES — DO NOT DESTROY RECURRENCE

When a task has `isRecurring: true` and a `due.string` like "every Thursday at 7:00pm":

- **Use `td task reschedule <ref> <date>` to postpone a recurring task.**
  It preserves the recurrence rule and only changes the next occurrence date.
  For a timed task, preserve its existing clock time with the full
  `YYYY-MM-DDTHH:MM:SS` format. A bare `YYYY-MM-DD` silently makes it all-day
  and deletes relative reminders; use date-only targets only for tasks that
  were already all-day.
  **Verified 2026-07-24**: "every Friday at 9pm" → rescheduled to Aug 7 at 9pm,
  `isRecurring` stayed `true`, `due.string` stayed "every Friday at 9pm".
- **NEVER use `td task update <ref> --due "2026-07-30"` with a bare date.**
  This replaces the `due.string` with the literal date, destroying recurrence.
  **Verified**: `--due "2026-07-31T23:00:00"` on a recurring task sets
  `isRecurring: false` and `due.string: "2026-07-31T23:00:00"`.
- **Re-applying `due.string` advances recurrence; it is not idempotent**:
  `td task update <ref> --due "<existing due.string>"` keeps the rule, time,
  and reminders, but advances an occurrence on every call. An overdue daily
  task can skip today even when today's time is still ahead. Use this only when
  the next recurrence strictly after today is wanted (for example, an overdue
  weekly task). For daily catch-up, use a full-datetime `reschedule` targeting
  today while preserving the task's existing time.
- **Before touching any recurring task**, always `td task view <ref> --json` first.
- **After any mutation on a recurring task**, re-fetch and verify `isRecurring`
  is still `true`.
