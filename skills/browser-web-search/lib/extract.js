...
export function parseHtmlToContent(html, url, truncate = true) {
  ...
  try {
    ...
    try {
      ...
      // cssstyle bug: some pages have CSS like "border: inherit" that crashes
      // the cssstyle normalize.js when JSDOM parses <style> tags.
      // Strip <style> tags and retry.
      const cleaned = html.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "");
      dom = new JSDOM(cleaned, { url, virtualConsole: jsdomVirtualConsole });
    }
  }
